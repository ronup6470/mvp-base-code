...
import {EmployeeAdapter,UserAdapter,CustomerAdapter, CustomerFilterAdapter, } from './user-info-adapter/user-info.adapter';
import { Employee, User, Customer, CustomerFilterRecord, } from './user-info.model';
...
export class UserInfoService {
   ...
   * @returns - user[]
   */
  public getUsers(tableProperty: TableProperty): Observable<User[]> {
    const url = this.baseUrl + 'User';
    const params = this.paramProcess(tableProperty);
    return this.http.httpGetRequest<User[]>(
      url,'1.0', { params: { ...params } }).pipe(map((data: User[]) => {
        return data.map((items) => this.userAdapter.toResponse(items));
      }));
  }
  ...
  public getUserById(id: string): Observable<User> {
    ...
    return this.http.httpGetRequest<User>(url,'1.0').pipe(map((response: User)=> this.userAdapter.toResponse(response)));;
  }

  /** This will save the record into database */
  public addUser(user: User): Observable<User> {
    const url = this.baseUrl + 'User';
    return this.http.httpPostRequest<User>(url, user, '1.0');
  }

    /** This will save the record by id into database */
  public updateUser(id: string,user: User): Observable<User> {
    const url = this.baseUrl + 'User/' + id;
    return this.http.httpPutRequest<User>(url, user, '1.0');
  }
  ...
  public deleteUser(user: User): Observable<User> {
    ...
    let options = {
      headers: new HttpHeaders(),
      body: user    };
    return this.http.httpDeleteRequest<User>(url, '1.0', options);
  }
